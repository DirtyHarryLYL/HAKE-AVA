...
def load_boxes_and_labels(cfg, mode):
    ...
    all_boxes, count, unique_box_count = parse_bboxes_file(
        ...
        num_parts=len(cfg.AVA.PASTA_PART_NAMES),
    )
    logger.info(
...
def parse_bboxes_file(
    ann_filenames, ann_is_gt_box, detect_thresh, boxes_sample_rate=1, num_parts=6,
):
    """
    ...
    for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box):
        with g_pathmgr.open(filename, "r") as f:
            for line in f:
                ...
                if len(row) > 8: # with pasta labels
                ...
                if len(row) > 8: # collect pasta labels
                    for part_idx in range(num_parts):
                        labels_per_part = list(map(int, pasta_labels[part_idx].split('_')))
                        for label_per_part in labels_per_part:
                            if label_per_part != -1:
                                all_boxes[video_name][frame_sec][box_key][2+part_idx].append(label_per_part)
