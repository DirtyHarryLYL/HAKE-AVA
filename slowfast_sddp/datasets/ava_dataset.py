...
class Ava(torch.utils.data.Dataset):
    ...
    def _images_and_boxes_preprocessing_cv2(self, imgs, boxes):
        ...
        if self._split == "train":  # "train"
            ...
            if not self.cfg.AVA.VAL_NO_CENTRE_CROP:
...
class Hake(Ava):
    ...
    def __init__(self, cfg, *args, **kwargs):
        ...
        self._pasta_classes = cfg.MODEL.PASTA_CLASSES
    ...
    def __getitem__(self, idx):
        ...
        for box_labels in clip_label_list:
            boxes.append(box_labels[0])
            labels.append(box_labels[1])
            for part_idx in range(len(self._pasta_classes)):
                pasta_labels[part_idx].append(box_labels[2 + part_idx])
        ...
        # Construct label arrays.
        label_arrs = np.zeros((len(labels), self._num_classes), dtype=np.int32)
        pasta_label_arrs = [np.zeros((len(pasta_labels[part_idx]), self._pasta_classes[part_idx]), dtype=np.int32) for part_idx in range(len(self._pasta_classes))]
        ...
        for i, box_labels in enumerate(labels):
            ...
            for label in box_labels:
                if label == -1:
                    continue
                assert label >= 1 and label <= 80
                label_arrs[i][label - 1] = 1 # AVA: 1 based

        for part_idx in range(6):
            for i, box_labels in enumerate(pasta_labels[part_idx]):
                for label in box_labels:
                    if label == -1:
                        continue
                    assert label >= 0
                    pasta_label_arrs[part_idx][i][label] = 1 # HAKE-Video: 0 based
        ...
        extra_data = {
            ...
            "ori_boxes": ori_boxes,
            "metadata": metadata,
            "pasta_label_arrs": pasta_label_arrs
        }
