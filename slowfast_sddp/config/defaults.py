...
def assert_and_infer_cfg(cfg):
    cfg.TRAIN.BATCH_SIZE = cfg.TRAIN.PER_GPU_BS * cfg.NUM_GPUS 
    ...
    # Execute LR scaling by total gpu number.
    if cfg.SOLVER.BASE_LR_SCALE_NUM_SHARDS:
        # cfg.SOLVER.BASE_LR *= cfg.NUM_SHARDS
        cfg.SOLVER.BASE_LR = cfg.SOLVER.BASE_LR * cfg.NUM_SHARDS * cfg.NUM_GPUS / 8

    # General assertions.
    assert cfg.SHARD_ID < cfg.NUM_SHARDS

    cfg.MODEL.NON_FROZEN_PASTAS = [pasta_idx for pasta_idx in range(len(cfg.AVA.PASTA_PART_NAMES)) \
                                   if cfg.AVA.PASTA_PART_NAMES[pasta_idx] not in cfg.MODEL.FROZEN_PASTAS]
    return cfg
