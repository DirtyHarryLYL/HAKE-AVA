...
import slowfast_sddp.datasets.ava_helper as ava_helper
...
from slowfast_sddp.utils.ava_eval_helper import (
    evaluate_ava,
    read_csv,
    read_csv_pasta,
    read_exclusions,
...
class AVAMeter(object):
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        if cfg.MODEL.PASTA_CLASSIFIER:
            self.pasta_categories, self.pasta_whitelist = read_labelmap(
                os.path.join(cfg.AVA.ANNOTATION_DIR, cfg.AVA.PASTA_LABEL_MAP_FILE)
            )
            # self.pasta_categories = [None] * len(cfg.AVA.PASTA_PART_NAMES)
            # for part_idx in range(len(cfg.AVA.PASTA_PART_NAMES)):
            #     categories, class_whitelist = read_labelmap(
            #         os.path.join(cfg.AVA.ANNOTATION_DIR, cfg.AVA.PASTA_LABEL_MAP_DIR, cfg.AVA.PASTA_PART_NAMES[part_idx]+'.pbtxt')
            #     )
            #     self.pasta_categories[part_idx] = [categories, class_whitelist]
        ...
        if cfg.MODEL.PASTA_CLASSIFIER:
            gt_filename = os.path.join(
                cfg.AVA.ANNOTATION_DIR, cfg.AVA.PASTA_GROUNDTRUTH_FILE
            )
            self.full_groundtruth_pasta = read_csv_pasta(gt_filename)
            self.mini_groundtruth_pasta = get_ava_mini_groundtruth(self.full_groundtruth_pasta)
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if self.mode == "train":
            stats = {
                ...
                "cur_iter": "{} / {} ({:.2f} %)".format(cur_iter + 1, self.overall_iters, 100 * (cur_iter+1) / self.overall_iters),
                "eta": eta,
    ...
    def update_stats(self, preds, ori_boxes, metadata, preds_pasta, loss=None, lr=None):
        """
        ...
        if self.mode in ["val", "test"]:
            ...
            if self.cfg.MODEL.PASTA_CLASSIFIER:
                self.all_preds_pasta.append(preds_pasta)
        if loss is not None:
            self.loss.add_value(loss)
        if lr is not None:
            self.lr = lr

    def finalize_metrics(self, log=False):
        """
        ...
        if self.cfg.MODEL.PASTA_CLASSIFIER:
            all_preds_pasta = torch.cat(self.all_preds_pasta, dim=0)
        ...
        self.pasta_per_part_map = [None] * len(self.cfg.AVA.PASTA_PART_NAMES)
        if self.cfg.MODEL.PASTA_CLASSIFIER:
            if self.mode == "test" or (self.full_ava_test and self.mode == "val"):
                groundtruth_pasta = self.full_groundtruth_pasta
            else:
                groundtruth_pasta = self.mini_groundtruth_pasta
            
            self.pasta_map, details = evaluate_ava(
                all_preds_pasta,
                all_ori_boxes,
                all_metadata.tolist(),
                self.excluded_keys,
                self.pasta_whitelist,
                self.pasta_categories,
                groundtruth=groundtruth_pasta,
                video_idx_to_name=self.video_idx_to_name,
                name="pasta"
            )
            
            for part_idx in range(len(self.cfg.AVA.PASTA_PART_NAMES)):
                part_name = self.cfg.AVA.PASTA_PART_NAMES[part_idx]
                stat = []
                for pasta_name, score in details.items():
                    if part_name+":" in pasta_name:
                        stat.append(score)
                mean_stat = np.nanmean(stat)
                self.pasta_per_part_map[part_idx] = mean_stat
            
            # self.pasta_map[part_idx] = pasta_map
        # if log:
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if self.mode in ["val", "test"]:
            ...
            if self.cfg.MODEL.PASTA_CLASSIFIER:
                stats['pasta_state_mean_ap'] = self.pasta_map
                for part_idx, part_name in enumerate(self.cfg.AVA.PASTA_PART_NAMES):
                    stats[f'pasta_part_mean_ap: {part_name}'] = self.pasta_per_part_map[part_idx]
                stats['pasta_part_mean_ap'] = np.nanmean(self.pasta_per_part_map)
            logging.log_json_stats(stats)
