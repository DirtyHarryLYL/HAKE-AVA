...
from slowfast_sddp.utils.ava_evaluation import (
...
def read_csv_pasta(csv_file, load_score=False, num_parts=6, pasta_num_classes=[15, 14, 5, 33, 7, 13]):
    """Loads boxes and class labels from a CSV file in the AVA format.
    CSV file format described at https://research.google.com/ava/download.html.
    Args:
      csv_file: A file object.
      class_whitelist: If provided, boxes corresponding to (integer) class labels
        not in this set are skipped.
      num_parts: The part number of PaSta data.
      pasta_num_classes: The number of PaSta classes for each part.
    Returns:
      boxes: A dictionary mapping each unique image key (string) to a list of
        boxes, given as coordinates [y1, x1, y2, x2].
      labels: A dictionary mapping each unique image key (string) to a list of
      ...
      scores: A dictionary mapping each unique image key (string) to a list of
        ...
        scores are not provided in the csv, then they will default to 1.0.
    """
    pasta_extend_ranges = np.cumsum(pasta_num_classes)
    ...
    with g_pathmgr.open(csv_file, "r") as f:
        ...
        for row in reader:
            ...
            if load_score:
                score = float(row[7])
            pasta_labels = list(row[8:14])
            to_append_pasta_label = [[], [], [], [], [], []]
            for part_idx in range(num_parts):
                labels_per_part = list(map(int, pasta_labels[part_idx].split('_'))) # 0 based
                for label_per_part in labels_per_part:
                    if label_per_part != -1:
                        to_append_pasta_label[part_idx].append(label_per_part)
            for part_idx in range(num_parts):
                for label_per_part in to_append_pasta_label[part_idx]:
                    if part_idx >= 1:
                        to_append = label_per_part + pasta_extend_ranges[part_idx - 1] # still use 0 based offset
                    else:
                        to_append = label_per_part
                    boxes[image_key].append([y1, x1, y2, x2])
                    scores[image_key].append(score)
                    labels[image_key].append(to_append + 1)  # convert data to 1 based, AVA/PaSta benchmark.
    return boxes, labels, scores
...
def evaluate_ava(
    ...
    # os.makedirs('ava_results', exist_ok=True)
    # write_results(detections, "ava_results/detections_%s.csv" % name)
    # write_results(groundtruth, "ava_results/groundtruth_%s.csv" % name)
